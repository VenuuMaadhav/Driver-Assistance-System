...
%% ANN 
target = label{1,:};
final_feature2 = [final_feature target];
inputs = final_feature2(:,1:end-1)';
targets = final_feature2(:,end)';
hiddenLayerSize = 10;
...
lab(66:121) = 4;
lab(122:246) = 5;
[C,order] = confusionmat(lab,yfit1);
% plotconfusion(lab,yfit1);
...
load yfit1.mat;
predi = yfit1;
EVAL1 = Evaluate(actual,predi);
% % Surf
load yfit12.mat;
pred12 = yfit1;
EVAL = Evaluate(actual,pred12);
% % % Create Table
LastName = {'Accuracy';'Sensitivity';'Specificity';'Precision';'Recall';'F-Measure';'G-mean'};
KNN_SURF = {EVAL(1);EVAL(2);EVAL(3);EVAL(4);EVAL(5);EVAL(6);EVAL(7)};
KNN_HARRIES = {EVAL1(1);EVAL1(2);EVAL1(3);EVAL1(4);EVAL1(5);EVAL1(6);EVAL1(7)};
T = table(KNN_SURF,KNN_HARRIES,...
    'RowNames',LastName);
disp(T)